...
6. Generate or import candidate: use `references/provider-output-contract.md`; default keypose counts, canvas, and background come from the active profile. For source animation, generate or repair only approved parts, then assemble keyposes. Use `add_secondary_motion_pass.py` only as targeted repair when liveliness is missing. For IPAdapter local repair, read `references/ipadapter-local-repair-route.md` and constrain edits to the part mask. For ToonCrafter interpolation, read `references/tooncrafter-interpolation-route.md` and use only approved keypose pairs. For Animate-X, read `references/animatex-provider-route.md`; use it only for de-identified large full-body video candidates.
   If the user only asks to preview rough whole-character motion before production layers exist, use `create_diagnostic_sequence_preview.py`; do not write one-off `generate_<character>_<action>_sequence.py` scripts for user-facing skill output. Do not use diagnostic preview for walk, run, step, tail, hand, prop, accessory, blink, or expression actions; those need source animation, clean masks/backfill, or provider keyposes.
...
For quick rough previews, use `scripts/create_diagnostic_sequence_preview.py`. Its output is always `diagnostic_only` and `admission_eligible: false`; it may help choose a whole-character bounce/idle rhythm, but it cannot enter production freeze, locked export, finalization, or admission. If the requested action requires feet, tail, hands, props, accessories, or expression changes, the diagnostic preview must fail instead of returning weak non-semantic frames.
